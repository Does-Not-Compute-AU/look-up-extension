...
const Account = ({ id, account: { attributes: { displayName, balance } }, transactions }) => {
                    ...
                    : (
                        <div className={"bg-white flex flex-row p-2"}>
                            ...
                            <div className={clsx("w-24 flex justify-end font-semibold")}>
                                <span className={tx?.attributes?.amount?.valueInBaseUnits > 0 ? "text-green-500" : ""}>{parseBaseUnit(tx?.attributes?.amount?.valueInBaseUnits)}</span>
